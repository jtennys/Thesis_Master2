...
;  Global Symbols
;-----------------------------------------------
export   RECEIVE_EnableInt
export  _RECEIVE_EnableInt
export   RECEIVE_DisableInt
export  _RECEIVE_DisableInt
export   RECEIVE_Start
export  _RECEIVE_Start
export   RECEIVE_Stop
export  _RECEIVE_Stop
export   RECEIVE_bReadRxData
export  _RECEIVE_bReadRxData
export   RECEIVE_bReadRxStatus
export  _RECEIVE_bReadRxStatus

; Old function name convension, do not use.
; These will be removed in a future release.
export  bRECEIVE_ReadRxData
export _bRECEIVE_ReadRxData
export  bRECEIVE_ReadRxStatus
export _bRECEIVE_ReadRxStatus

;-----------------------------------------------
; High Level RX functions
;-----------------------------------------------

export  RECEIVE_cGetChar
export _RECEIVE_cGetChar
export  RECEIVE_cReadChar
export _RECEIVE_cReadChar
export  RECEIVE_iReadChar
export _RECEIVE_iReadChar

IF (RECEIVE_RXBUF_ENABLE)
export  RECEIVE_CmdReset
export _RECEIVE_CmdReset
export  RECEIVE_bCmdCheck
export _RECEIVE_bCmdCheck
export  RECEIVE_bCmdLength
export _RECEIVE_bCmdLength
export  RECEIVE_bErrCheck
export _RECEIVE_bErrCheck

export  RECEIVE_szGetParam
export _RECEIVE_szGetParam
export  RECEIVE_szGetRestOfParams
export _RECEIVE_szGetRestOfParams
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_EnableInt
;
;  DESCRIPTION:
...
_RECEIVE_EnableInt:
   ...
   M8C_EnableIntMask RECEIVE_INT_REG, RECEIVE_bINT_MASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_DisableInt
;
;  DESCRIPTION:
...
_RECEIVE_DisableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   M8C_DisableIntMask RECEIVE_INT_REG, RECEIVE_bINT_MASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_Start(BYTE bParity)
;
;  DESCRIPTION:
...
_RECEIVE_Stop:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   REG[RECEIVE_CONTROL_REG], ~bfCONTROL_REG_START_BIT
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_bReadRxData
;
;  DESCRIPTION:
...
;
 RECEIVE_bReadRxData:
_RECEIVE_bReadRxData:
 bRECEIVE_ReadRxData:
_bRECEIVE_ReadRxData:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A, REG[RECEIVE_RX_BUFFER_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_bReadRxStatus
;
;  DESCRIPTION:
...
;    functions.
;
 RECEIVE_bReadRxStatus:
_RECEIVE_bReadRxStatus:
 bRECEIVE_ReadRxStatus:
_bRECEIVE_ReadRxStatus:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A,  REG[RECEIVE_CONTROL_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_cGetChar
;
;  DESCRIPTION:
...
.getChar_Loop:
   tst REG[RECEIVE_CONTROL_REG],RECEIVE_RX_REG_FULL   ; Check if a character is ready
   jz  .getChar_Loop                                        ; If not loop

   mov A, REG[RECEIVE_RX_BUFFER_REG]             ; Get character
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_cReadChar
;
;  DESCRIPTION:
;     Read character from UART RX port.
;
...
_RECEIVE_cReadChar:
   ...
   mov  A,REG[RECEIVE_CONTROL_REG]                         ; Get Status of RX
   push A
   and  A,RECEIVE_RX_COMPLETE                              ; Check if a character is ready
   jnz  .RX_DATA_RDY                                       ; Data Ready go read it.
   pop  A
   jmp  .RX_NO_VALID_CHAR

.RX_DATA_RDY:
   mov  A,REG[RECEIVE_RX_BUFFER_REG]                       ; Read data first, then
   swap A,X                                                ; determine if data is valid

   pop  A                                                  ; Check for errors
   and  A,(RECEIVE_RX_PARITY_ERROR | RECEIVE_RX_FRAMING_ERROR)
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_iReadChar
;
...
_RECEIVE_iReadChar:
   ...
   and  A,(RECEIVE_RX_ERROR|RECEIVE_RX_REG_FULL)
   push A
   and  A,RECEIVE_RX_COMPLETE                              ; Check if a character is ready
   jnz  .RX_GET_DATA                                       ; Data Ready go read it.
   pop  A
   or   A,RECEIVE_RX_NO_DATA                               ; Add no data flag
   swap A,X
   jmp  End_RECEIVE_iReadChar
...
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_CmdReset
;
;  DESCRIPTION:
...
_RECEIVE_CmdReset:
   ...
   RAM_SETPAGE_CUR >RECEIVE_aRxBuffer
   mov [RECEIVE_aRxBuffer], 0x00
   RAM_SETPAGE_CUR >ptrParam
   mov [ptrParam],0x00
   RAM_SETPAGE_CUR >RECEIVE_bRxCnt
   mov [RECEIVE_bRxCnt], 0x00
   and [RECEIVE_fStatus], 0x00
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_bCmdCheck
;
;  DESCRIPTION:
...
_RECEIVE_bCmdCheck:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >RECEIVE_fStatus
   mov A,  [RECEIVE_fStatus]
   and A, RECEIVE_RX_BUF_CMDTERM                 ; Mask off Command status
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_bErrCheck
;
;  DESCRIPTION:
...
_RECEIVE_bErrCheck:
   ...
   RAM_SETPAGE_CUR >RECEIVE_fStatus
   mov A,  [RECEIVE_fStatus]
   and A, RECEIVE_RX_BUF_ERROR                   ; Mask off Error status
   and [RECEIVE_fStatus], ~RECEIVE_RX_BUF_ERROR
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_bCmdLength
;
;  DESCRIPTION:
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_szGetParam
;
;  DESCRIPTION:
...
;     RECEIVE_CmdReset which resets the entire buffer to the 'empty'
;     state. Typically this function, RECEIVE_szGetParam, is
;     not called until the buffer has been loaded with an entire command
;     (See RECEIVE_bCmdCheck).
;
...
_RECEIVE_szGetParam:
   ...
   mov  A, <RECEIVE_aRxBuffer               ; Get address to receive buffer
...
.CheckForDelim:
    cmp  A,RECEIVE_DELIMITER                ; Check if we have a delimiter
    jnz  .ParamStartFound
    inc  X                                  ; Increment both current pointer and
    inc  [ptrParam]                         ; stored pointer.
    mov  A,[X]                              ; Get character pointed by X
    cmp  [ptrParam],(RECEIVE_RX_BUFFER_SIZE -1)  ; Check if we are at the end of buffer
    jnz  .CheckForDelim
                                            ; End of string found
.EndOfString:
    push X                                  ; Save ptr
...
.ParamLoop:
    ...
    mov  [ptrParam],(RECEIVE_RX_BUFFER_SIZE -1)  ; Reset pointer to end of string.
...
.NotNullString:
   pop  X
   mov  A,>RECEIVE_aRxBuffer                     ; Return pointer
   RAM_EPILOGUE RAM_USE_CLASS_3
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_szGetRestOfParams
;
;  DESCRIPTION:
...
;          CUR_PP
;
 RECEIVE_szGetRestOfParams:
_RECEIVE_szGetRestOfParams:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >ptrParam
